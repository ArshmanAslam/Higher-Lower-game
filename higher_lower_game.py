...
def compare(pick1 , pick2):
  person1 = pick1['follower_count']
  person2 = pick2['follower_count']
  if person1 > person2 :
    return 'a'
  else:
    return 'b'

def print_personality(pick):
  name = pick['name']
  profession = pick['description']
...
while True:
  ...
  print(f"Compare A : {print_personality(person1)}")
  print(vs)
  ...
  if user == result:
    ...
    print(f"Wrong Guessed Your Final Score : {score}")
    break
